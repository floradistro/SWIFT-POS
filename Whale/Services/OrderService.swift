...
enum OrderService {
    ...
    static func fetchOrderForPrinting(orderId: UUID) async throws -> OrderPrintData? {
        ...
        if let jsonString = String(data: response.data, encoding: .utf8) {
            Log.network.debug("RPC raw response: \(jsonString.prefix(500))")
        }
        ...
        do {
            ...
            if let decodingError = error as? DecodingError {
                ...
                case .keyNotFound(let key, let context):
                    Log.network.error("Missing key: \(key.stringValue) at path: \(context.codingPath.map { $0.stringValue }.joined(separator: "."))")
                case .typeMismatch(let type, let context):
                    Log.network.error("Type mismatch for type: \(type) at path: \(context.codingPath.map { $0.stringValue }.joined(separator: "."))")
                case .valueNotFound(let type, let context):
                    Log.network.error("Value not found for type: \(type) at path: \(context.codingPath.map { $0.stringValue }.joined(separator: "."))")
                case .dataCorrupted(let context):
                    Log.network.error("Data corrupted at path: \(context.codingPath.map { $0.stringValue }.joined(separator: "."))")
                @unknown default:
                    Log.network.error("Unknown decoding error: \(error)")
                }
            }
            throw error
        }
